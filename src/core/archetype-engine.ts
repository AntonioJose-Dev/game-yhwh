 ...
 */
export function initializeArchetypeState(): ArchetypeState {
  const allNames: ArchetypeName[] = Object.keys(ARCHETYPE_CONFIGS) as ArchetypeName[];
  const now = Date.now();
  
  const scores: Record<ArchetypeName, ArchetypeScore> = {} as Record<ArchetypeName, ArchetypeScore>;
...
export function calculateDecisionImpact(
  ...
  const impact: Partial<Record<ArchetypeName, number>> = {};
  const allNames: ArchetypeName[] = Object.keys(ARCHETYPE_CONFIGS) as ArchetypeName[];
 ...
 */
export function updateArchetypeState(
  currentState: ArchetypeState,
  ...
  for (const [archetypeName, scoreDelta] of Object.entries(decision.archetypeScores)) {
    const name = archetypeName as ArchetypeName;
 ...
 */
export function calculateDominantArchetype(
...
export function createDecisionRecord(
  ...
  kingdom?: string,
  context?: string
): { decision: DecisionRecord; archetypeScores: Partial<Record<ArchetypeName, number>> } {
 ...
 */
export function revealTrueName(state: ArchetypeState): { 
 ...
 */
export function getArchetypeStatistics(state: ArchetypeState): {
  totalDecisions: number;
  topThree: Array<{ name: ArchetypeName; score: number; title: string }>;
  averageScore: number;
  mostActiveArchetype: ArchetypeName;
  leastActiveArchetype: ArchetypeName;
} {
 ...
 */
export function isCloseToArchetype(
  state: ArchetypeState,
  targetArchetype: ArchetypeName,
 ...
 */
export function getMostFrequentDecisionTypes(
  decisionLog: DecisionRecord[],
  limit: number = 3
): Array<{ type: DecisionType; count: number }> {
  const counts: Record<DecisionType, number> = {} as Record<DecisionType, number>;
  ...
  return Object.entries(counts)
    .map(([type, count]) => ({ type: type as DecisionType, count }))
