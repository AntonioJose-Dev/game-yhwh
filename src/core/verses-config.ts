...
import { DecisionType } from '../types/archetypes';

/**
 * Categorías de versículos
 */
export type VerseCategory = 
  | 'PROTECCION'      // Versículos defensivos (Armadura de Efesios)
  | 'ATAQUE'          // Versículos ofensivos directos
  | 'SANACION'        // Versículos de curación
  | 'SABIDURIA'       // Versículos que revelan debilidades
  | 'FE_PODEROSA'     // Versículos que invocan intervención divina
  | 'LIBERACION'      // Versículos contra ataduras espirituales
  | 'VALOR'           // Versículos que fortalecen el espíritu;

/**
 * Tipos de enemigos del bestiario espiritual
 */
...
/**
 * Efecto principal del versículo
 */
export interface VerseEffect {
  baseDamage: number;        // Daño base (0 si es solo efecto)
  feScaling: number;         // Multiplicador por nivel de FE (0.5 = 50% del FE actual)
  healAmount?: number;       // Cantidad de curación (si aplica)
  revealWeakness: boolean;   // Si revela debilidad del enemigo
  divineIntervention: boolean; // Si invoca intervención directa de YHWH
  bonusDamageVs?: EnemyType[]; // Daño extra contra tipos específicos
}

/**
 * Configuración de un versículo individual
 */
export interface VerseConfig {
  ...
  effect: VerseEffect;
  unlockRequirement: {
    minStage: number;      // Etapa mínima para desbloquear
    minFeLevel?: number;   // Nivel mínimo de FE (opcional)
    kingdom?: string;      // Reino específico (opcional)
  };
  decisionTypeReward?: DecisionType; // Tipo de decisión que otorga este versículo
  cooldownTurns?: number;  // Turnos de recarga (0 = sin cooldown)
  manaCost?: number;       // Costo de maná (0 = gratis)
}

/**
 * Bestiario de enemigos con sus debilidades
 */
export interface EnemyBestiaryEntry {
  ...
  description: string;
  baseHp: number;
  weaknesses: VerseCategory[];  // Categorías efectivas contra este enemigo
  resistances: VerseCategory[]; // Categorías menos efectivas
  specialAbility?: string;      // Habilidad especial del enemigo
}

/**
 * Colección completa de versículos organizados
 */
export const VERSES_CONFIG: Record<string, VerseConfig> = {
 ...
 */
export const ENEMY_BESTIARY: Record<EnemyType, EnemyBestiaryEntry> = {
  'ORGULLO_SOBERBIO': {
 ...
 */
export function getAllVerses(): VerseConfig[] {
 ...
 */
export function getVerseById(verseId: string): VerseConfig | null {
 ...
 */
export function getUnlockedVerses(
  currentStage: number,
  feLevel: number,
  kingdom?: string
): VerseConfig[] {
 ...
 */
export function calculateVerseDamage(
  verse: VerseConfig,
  enemyType: EnemyType,
  feLevel: number
): number {
 ...
 */
export function getVerseEffectiveness(
  verse: VerseConfig,
  enemyType: EnemyType
): 'SUPER_EFFECTIVE' | 'EFFECTIVE' | 'NOT_EFFECTIVE' | 'RESISTED' {
  const enemy = ENEMY_BESTIARY[enemyType];
