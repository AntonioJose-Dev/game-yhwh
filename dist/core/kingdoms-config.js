 ...
 */
export function getKingdomForStage(stageId) {
    for (const kingdom of Object.values(KINGDOMS_CONFIG)) {
        const stage = kingdom.stages.find(s => s.id === stageId);
        if (stage)
            return kingdom;
 ...
 */
export function calculateStageStatus(currentStage, completedStages) {
 ...
 */
export function isKingdomLiberated(kingdomId, completedStages) {
    const kingdom = KINGDOMS_CONFIG[kingdomId];
    if (!kingdom)
        return false;
    return kingdom.stages.every(stage => completedStages.includes(stage.id));
}
/**
 * Obtiene el próximo reino disponible
 */
export function getNextAvailableKingdom(completedStages) {
...
//# sourceMappingURL=kingdoms-config.js.map
