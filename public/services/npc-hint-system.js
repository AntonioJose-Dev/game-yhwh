 ...
 */
export const NPC_HINTS_DATABASE = [
    // === PISTAS DE SABIDURÍA ===
    {
        id: 'wisdom_001',
        npcId: 'salomon',
 ...
 */
export function checkNPCHints(npcId, decisionLog, hintsState) {
    ...
    for (const hint of npcHints) {
        ...
        const matchingDecisions = decisionLog.filter(d => d.decisionType === hint.triggerCondition.decisionType);
        ...
        if (matchingDecisions.length >= hint.triggerCondition.minCount) {
            ...
            if (hint.triggerCondition.kingdom) {
                const kingdomDecisions = matchingDecisions.filter(d => d.kingdom === hint.triggerCondition.kingdom);
 ...
 */
export function markHintAsPlayed(hintsState, hintId, timestamp = Date.now()) {
 ...
 */
export function getAllAvailableHints(decisionLog, hintsState) {
...
//# sourceMappingURL=npc-hint-system.js.map